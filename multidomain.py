...
def bernoulli_kl_div(student_probs, teacher_probs):
    """Compute KL(teacher || student) for independent Bernoulli pixels."""
    student_dist = torch.stack((student_probs, 1 - student_probs), dim=-1)
    teacher_dist = torch.stack((teacher_probs, 1 - teacher_probs), dim=-1)
...
def kd_train(student_model, loader, pseudo_labels, optimizer, device, epochs=5, batch_size=8, alpha=1e-4):
    ...
    for epoch in range(epochs):
        ...
        for i in tqdm(range(0, len(dataset), batch_size), desc=f"KD-Train epoch {epoch+1}"):
            ...
            loss_kl = bernoulli_kl_div(student_probs, soft_labels)
